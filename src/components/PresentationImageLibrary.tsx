...
export default function PresentationImageLibrary({ images, selectedId, onSelect, label = 'Images & backgrounds' }: { images: PresentationImage[]; selectedId?: string; onSelect?: (image: PresentationImage) => void; label?: string }) {
  ...
  return <><button type="button" className="min-h-11 rounded-xl border border-[var(--line)] bg-white px-4 py-2 text-sm font-semibold text-[var(--green)]" onClick={() => setOpen(true)}>{label}</button>{open && createPortal(<Gallery images={images} selectedId={selectedId} onSelect={onSelect} onClose={close} />, document.body)}</>;
}
