...
type Props = { lesson: Omit<LessonSource, 'scripture'>; resolveScripture: (passage: string) => string };
...
function Builder({ lesson, resolveScripture, onClose }: Props & { onClose: () => void }) {
  ...
  return <div ref={shell} className={styles.shell} role="dialog" aria-modal="true" aria-label="Pre-class countdown builder">
    ...
    <div className={styles.layout}><section className={styles.editor}>
      <h2>Build your countdown</h2><p>Saved separately from your lesson, on this browser. Audio must be selected each session.</p>
      ...
      {source.lessonPlan?.media && <p>Lesson media notes: {source.lessonPlan.media}</p>}
      <ol className={styles.sequence}>{plan.items.map((item, i) => <li key={item.id}><button className={selected === i ? styles.active : ''} onClick={() => { setSelected(i); reset(); }}>{i + 1}. {item.title || item.kind}<small>{item.seconds}s{item.kind === 'question' ? ` + ${item.revealSeconds}s answer` : ''}</small></button></li>)}</ol>
      <div className={styles.actions}>{(['question','verse','context','phone','announcement','prayer'] as const).map(kind => <button key={kind} disabled={plan.items.length >= 40} onClick={() => { change({ items: [...plan.items, newItem(kind, kind === 'phone' ? 'Please silence your phone' : `New ${kind}`)] }); setSelected(plan.items.length); reset(); }}>Add {kind}</button>)}</div>
      {current && <div className={styles.card}>
      ...
      <label><input type="checkbox" checked={plan.reviewed} onChange={e => { setPlan({ ...plan, reviewed: e.target.checked }); setDirty(true); }} /> I reviewed the answers, KJV wording, doctrine, announcements, and permission to display this content.</label>
      </fieldset>
    ...
    </section><section className={styles.preview}>
      <div ref={stage} className={`${styles.stage} ${presenting ? styles.presenting : ''}`} aria-label="Audience preview" onDoubleClick={() => { if(presenting){stop();setPresenting(false);if(document.fullscreenElement)void document.exitFullscreen();} }}>
