...
Update on 2026-06-22: a full MetaV staging helper now exists:

```bash
npm run prepare:tsk-metav-full
```

It downloads `Verses.csv` and `CrossRefIndex.csv` from the MetaV GitHub source,
converts them to the app's TSK row shape, and writes review chunks under
`data/staging/tsk/metav-full/`. That folder is intentionally ignored by Git.
Nothing generated by this command becomes public. Use it to review and promote
small verified batches, not to ship the full Creative Commons ShareAlike dataset
silently.

For a small test batch:

```bash
npm run prepare:tsk-metav-full -- --limit=300 --chunk-size=100
```

For a full local staging run:

```bash
npm run prepare:tsk-metav-full -- --chunk-size=2500
```

Promotion rule: copy only reviewed rows into a tracked `data/imports/*reviewed*.json`
file, change source metadata if a purely public-domain source replaces MetaV,
then run `npm run validate:tsk -- <reviewed-file>` before it can appear in the
public app.
