# Dictionary Coverage Audit

Generated: 2026-07-16T02:19:18.564Z

This report checks the app's common study words against the structured Webster 1828 data and text-searchable Easton, Smith, and Nave resources. Easton/Smith/Nave are not yet parsed into normalized dictionary entries, so their matches are text-presence checks.

## Summary

- Study words checked: 75
- Webster exact coverage: 75/75 (100%)
- Easton text coverage: 72/75 (96%)
- Smith text coverage: 75/75 (100%)
- Nave text coverage: 74/75 (98.7%)

## Webster Missing Exact Headwords

- None

## Next Work

- Parse Easton, Smith, and Nave into normalized headword/topic indexes.
- Add reviewed aliases for KJV forms that point to existing dictionary headwords.
- Keep Webster overlays small and reviewed; do not use OCR guesses for doctrine-critical definitions.

## Coverage Rows

| Word | Webster | Easton text | Smith text | Nave text |
| --- | --- | --- | --- | --- |
| adoption | yes | yes | yes | yes |
| beast | yes | yes | yes | yes |
| believe | yes | yes | yes | yes |
| boast | yes | yes | yes | yes |
| born | yes | yes | yes | yes |
| charity | yes | yes | yes | yes |
| condemned | yes | yes | yes | yes |
| controversy | yes | yes | yes | yes |
| covenant | yes | yes | yes | yes |
| death | yes | yes | yes | yes |
| disciple | yes | yes | yes | yes |
| dominion | yes | yes | yes | yes |
| dove | yes | yes | yes | yes |
| faith | yes | yes | yes | yes |
| flesh | yes | yes | yes | yes |
| flock | yes | yes | yes | yes |
| follow | yes | yes | yes | yes |
| glory | yes | yes | yes | yes |
| grace | yes | yes | yes | yes |
| groaning | yes | yes | yes | yes |
| heal | yes | yes | yes | yes |
| hearken | yes | yes | yes | yes |
| heart | yes | yes | yes | yes |
| holy | yes | yes | yes | yes |
| intercede | yes | no | yes | no |
| judgment | yes | yes | yes | yes |
| kingdom | yes | yes | yes | yes |
| know | yes | yes | yes | yes |
| knowledge | yes | yes | yes | yes |
| law | yes | yes | yes | yes |
| life | yes | yes | yes | yes |
| light | yes | yes | yes | yes |
| love | yes | yes | yes | yes |
| mark | yes | yes | yes | yes |
| mercy | yes | yes | yes | yes |
| mission | yes | yes | yes | yes |
| mix | yes | yes | yes | yes |
| pastor | yes | yes | yes | yes |
| perish | yes | yes | yes | yes |
| praise | yes | yes | yes | yes |
| prayer | yes | yes | yes | yes |
| preach | yes | yes | yes | yes |
| pride | yes | yes | yes | yes |
| promise | yes | yes | yes | yes |
| prophecy | yes | yes | yes | yes |
| prophet | yes | yes | yes | yes |
| recompense | yes | yes | yes | yes |
| repentance | yes | yes | yes | yes |
| resurrection | yes | yes | yes | yes |
| return | yes | yes | yes | yes |
| righteousness | yes | yes | yes | yes |
| sacrifice | yes | yes | yes | yes |
| salvation | yes | yes | yes | yes |
| sanctification | yes | yes | yes | yes |
| sanctify | yes | no | yes | yes |
| saved | yes | yes | yes | yes |
| shepherd | yes | yes | yes | yes |
| sin | yes | yes | yes | yes |
| sow | yes | yes | yes | yes |
| spirit | yes | yes | yes | yes |
| supplication | yes | yes | yes | yes |
| teach | yes | yes | yes | yes |
| testament | yes | yes | yes | yes |
| thanksgiving | yes | yes | yes | yes |
| transgression | yes | yes | yes | yes |
| truth | yes | yes | yes | yes |
| understanding | yes | yes | yes | yes |
| vision | yes | yes | yes | yes |
| visitation | yes | yes | yes | yes |
| whoredom | yes | no | yes | yes |
| wisdom | yes | yes | yes | yes |
| witness | yes | yes | yes | yes |
| world | yes | yes | yes | yes |
| worship | yes | yes | yes | yes |
| zion | yes | yes | yes | yes |
