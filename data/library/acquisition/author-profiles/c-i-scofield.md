# C. I. Scofield Acquisition Profile Stub

## Do Not Use Yet

This author/profile is for acquisition review only. No content may be imported, displayed, summarized, embedded, or recommended until copyright status, permission status, and theological review are complete.

## Review Status

| Field | Value |
| --- | --- |
| Author / ministry | C. I. Scofield |
| Resource category | Study Bible notes and dispensational helps |
| Profile status | Needs Review |
| Resource approval status | Needs Review |
| Copyright status | Unknown - verify |
| Permission needed | Maybe |
| Permission/contact info | TBD |
| Website | TBD |
| Collection label | Needs Review |
| Review score | Not scored |
| License obtained | No |

## Biography

Placeholder. Add a short verified biography after source review.

Include:

- birth and death years, if applicable
- ministry background
- major ministry work
- theological tradition
- publication history
- source citations

## Doctrinal Notes

Placeholder. Compare this author, ministry, or publisher against `DOCTRINAL_BASIS.md` and `RESOURCE_VETTING_GUIDELINES.md` before recommending any resource.

Review:

- Gospel clarity
- KJV posture
- Baptist compatibility
- dispensational compatibility
- church doctrine
- prophecy handling
- Christian living emphasis
- any areas where readers should understand differing viewpoints

## Major Works / Resource Candidates

| Title | Type | Year | Rights status | Review status | Notes |
| --- | --- | --- | --- | --- | --- |
| TBD | Study Bible notes and dispensational helps | TBD | Unknown - verify | Needs Review | Add title-level review before import. |

## Audio / Video Availability

| Media type | Availability | Rights / permission status | Notes |
| --- | --- | --- | --- |
| Audio | Unknown | Needs Review | Do not host, transcribe, summarize, or generate narration without permission. |
| Video | Unknown | Needs Review | Official links or embeds require documented permission. |

## Permission And Contact Notes

| Field | Value |
| --- | --- |
| Known contact | TBD |
| Website | TBD |
| Permission status | Needs Review |
| Requested use | TBD |
| Follow-up needed | Yes |

Notes:

- Do not approve broad catalog use.
- Ask about specific titles, excerpts, links, audio, video, transcript use, image/chart use, and future paid-access terms separately.
- Record written permission before any copyrighted material becomes public in the app.
- Public-domain candidates still require source, edition, and OCR/text-quality review.

## Review Score

Scores use 1-5. Leave blank until reviewed.

| Category | Score | Notes |
| --- | --- | --- |
| Biblical Accuracy |  |  |
| Gospel Clarity |  |  |
| Scripture Handling |  |  |
| KJV Friendliness |  |  |
| Baptist Compatibility |  |  |
| Dispensational Compatibility |  |  |
| Practical Value |  |  |
| Historical Importance |  |  |
| Source Quality |  |  |
| Rights Confidence |  |  |
| Overall Recommendation |  |  |

## Baptist / KJV / Dispensational Fit Notes

Placeholder. Add notes after review.

Current intake note:

> 1917 materials may be public-domain candidates; later editions and notes require review.

## Final Recommendation

Needs Review. No author, ministry, publisher, book, sermon, audio, video, quote, chart, transcript, or excerpt is approved by this stub.
